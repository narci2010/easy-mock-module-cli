...
async function init (apis, cb) {
  ...
  projectIds.forEach(projectId => {
    ...
    for (let i = 0; i < swaggerapis.length; i++) {
      ...
      let modulePath = apiItem.url.split(/\//)[1]
      ...
      // 如果模块名有特殊字符，去掉
      modulePath = modulePath.replace(/[\.\'\"\\\/\b\f\n\r\t]/g, '')
      moduleNames.push(modulePath)
      moduleNames = _.uniq(moduleNames)

      // logger.log('> init api2 %s', api2.length)
      // + '/' + modulePath
      var oldDest = path.join(config.output, confProject.name)
      var dest = oldDest
      if (!_.isEmpty(modulePath)) {
        dest = oldDest + '/' + modulePath
      }

      // var dest = path.join(config.output, confProject.name)
      var data = {
        data: { project: api.project, mocks: api2, moduleNames: moduleNames },
        config: config,
        project: confProject,
        _: _,
